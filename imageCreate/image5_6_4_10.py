"""
h = h + Log(Abs(Sin(x * Cos(2 * pi * i / q) + y * Sin(2 * pi * i / q))))
+ Cos(x * Cos(2 * pi * i / q) + y * Sin(2 * pi * i / q)) ^ 100
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_6_4_10 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 80)
