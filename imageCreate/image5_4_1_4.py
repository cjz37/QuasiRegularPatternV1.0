"""
h = h + np.cos(x * np.exp(np.sqrt(np.abs(coslist[i]))) + y * np.exp(np.sqrt(np.abs(sinlist[i]))))
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_10_4 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 46)
