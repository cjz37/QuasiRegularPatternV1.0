"""
h = h + Cos(x * Cos(2 * pi * i / q) + y * Sin(2 * pi * i / q)) + Cos(x * Cos(2 * pi * i / q)
+ y * Sin(2 * pi * i / q)) ^ 100 + Cos(x * Cos(2 * pi * i / q) + y * Sin(2 * pi * i / q)) ^ 101
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_6_3_4 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 65)
