"""
h = h + np.cos(np.sin(np.abs(x) ^ 0.5) * coslist[i] ^ 2 + np.cos(np.abs(y) ^ 0.5) * sinlist[i] ^ 2)
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_9_5 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 42)
