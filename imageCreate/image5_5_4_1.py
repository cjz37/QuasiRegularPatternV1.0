"""
h = h + np.log(np.abs(np.cos(np.cos(np.log(np.abs(x))) * coslist[i] + np.log(np.abs(y)) * sinlist[i])))
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_5_4_1 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 54)
