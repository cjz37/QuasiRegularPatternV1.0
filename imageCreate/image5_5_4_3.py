"""
h = h + np.cos(np.log(np.abs(x)) * np.log(np.abs(coslist[i])) + np.log(np.abs(y)) * np.log(np.abs(sinlist[i])))
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_5_4_3 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 56)
