"""
h = h + Exp(Cos(x * Sin(Cos(2 * pi * i / q)) + y * Sin(Sin(2 * pi * i / q))))
+ Sqr(Abs(Cos(x * Cos(2 * pi * i / q) + y * Sin(2 * pi * i / q))))
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_8_1_1 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 91)
