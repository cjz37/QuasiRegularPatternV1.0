"""
h = h + np.exp(np.sqrt(np.abs(np.exp(np.cos(x * np.exp(coslist[i]) + y * np.exp(sinlist[i]))))))
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_10_6 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 48)
