"""
h = h + np.cos(np.sin(np.abs(x) ^ 0.5) * coslist[i] + np.cos(np.abs(y) ^ 0.5) * sinlist[i])
"""
...
def draw(q, s, w, xmin, ymin):
    ...
    name = '5_8_4 {} {}pi'.format(q, s)
    di.draw_image(q, s, w, color_index, name, xmin, ymin, mag, color_list, split_point, 37)
